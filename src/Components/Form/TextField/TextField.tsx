...
const StyledInput = styled.input<{ required?: boolean }>`
  ...
  &:hover, &:active {
    background: ${({ theme }) => hexToRGBA(theme.bme.palette.dark, .13)};
  }
...
const TextField: React.FunctionComponent<TextFieldComponentProps> = ({ name, value, label, characterLimit, helperText, errorText, onChange, required, valid }) => {

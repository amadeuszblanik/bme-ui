...
import {
  ...
  CardSubcomponents, CardThumbnailComponentProps
} from "./Card.types";
import {hexToRGBA} from "../../Mixins";

const StyledCard = styled(BMEBox)<CardStyledComponentProps>`
  overflow: hidden;
  border: ${({borderColour, theme}) => hexToRGBA(theme.bme.palette[borderColour], .33)} 1px solid;
`;

const StyledCardHeader = styled(BMEBox)<CardStyledComponentProps>`
  border-bottom: ${({borderColour, theme}) => hexToRGBA(theme.bme.palette[borderColour], .33)} 1px solid;
`
...
const CardFooter: React.FunctionComponent<CardStyledSubComponentProps> = ({ children }) => (
  <StyledCardFooter padding={{x: "m", y: "s"}}>
    {children}
  </StyledCardFooter>
)

const Card: React.FunctionComponent<CardComponentProps> & CardSubcomponents = ({children, borderColour, background}) => {
